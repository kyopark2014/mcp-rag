...
with st.sidebar:
    ...
    # radio selection
    mode = st.radio(
        label="원하는 대화 형태를 선택하세요. ",options=["일상적인 대화", "RAG", "Agent", 'Agent (Chat)', "번역하기 (한국어 / 영어)", "문법 검토하기", "이미지 분석", "카메라로 사진 찍어 번역하기", "비용 분석"], index=2
    )   
    ...
    modelName = st.selectbox(
        '🖊️ 사용 모델을 선택하세요',
        ("Claude 4.5 Haiku", "Claude 4.5 Sonnet", "Claude 4 Sonnet", "Claude 4 Opus", 'Claude 3.7 Sonnet', 'Claude 3.5 Sonnet', 'Claude 3.0 Sonnet', 'Claude 3.5 Haiku', "Nova Premier", 'Nova Pro', 'Nova Lite', 'Nova Micro'), index=index
    )
    ...
    if mode=='Agent' or mode=='Agent (Chat)':
        ...
        mcp_options = [ 
            "Basic", "MCP Knowledge Base Retriever", "AWS MCP (Knowledge Base)", "MCP Lambda (Knowledge Base)", "OpenSearch MCP", "MCP Lambda (OpenSearch)", "agentcore gateway", "사용자 설정"
        ]
        mcp_selections = {}
